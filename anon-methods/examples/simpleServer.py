...
""" server.py listens for incoming requests from the client and
extracts the parameters from the url sent by the client.
It then writes the extracted parameters to a file in JSON format and
waits for the Uber Tool to write the noisy result to a file.
Finally, it reads the result file and sends the noisy result along
with the Session ID back to the client.
"""
...
def read_file():
    ...
    if (file_count != temp_file_count):
        ...
        print("\nLast modified file: " + latest_file)  # Print last modified filename
...
This method is called initially when SessionID is NULL and it creates 
a file with the Session ID assigned to the particular client request.
"""
...
"""
Method to update the client request if the client has sent a request previously
i.e., SID is already present. If the SID is already present in one of the previously
written files, it will update the file with new queries and used budget values.
"""
...
Returns the query result (Noisy Result) as response to Client along with Session ID (sid).
"""
...
class GetParams(Resource):
    def get(self):
        ...
        if not sid:
            ...
            # JSON result to be sent back to the client
            result_json = {
                "Server Response": {
                    "Result": result[0],
                    "Session ID": sid
        ...
        # open the existing file created from previous session by comparing 'sid'
            ...
            if (float(updated_used_budget) <= float(budget)):
                ...
                # If an Error occurs while processing the query then send JSON containing error
                if 'Error'.lower() in result[0].lower():
                    ...
                    print(result_json)
                    return result_json  # Return the error and Session ID to client

                # If there is no error then send the query result and Session ID to client
                else:
                    result_json = {
                        "Server Response": {
                            "Result": result[0],
            ...
            # If used budget value exceeds the original budget then send error message
            else:
                ...
                return result_json  # Return the error message stating budget has exceeded
